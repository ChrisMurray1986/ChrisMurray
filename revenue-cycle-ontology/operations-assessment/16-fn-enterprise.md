...
| PFM-14.2-02 | Clinical-system build ships with no revenue-cycle seat (14.2.A6, D1) — order sets, visit types, scheduling trees, and documentation templates change revenue surfaces silently; impact discovered as downstream denial/edit spikes | Order-set changes reach revenue cycle as denial clusters weeks later; no RC sign-off in clinical change control; visit-type→auth-grid sync manual or absent | PBP-14.2-02 Revenue-impact review embedded in clinical change control with regression on charge triggers, auth mappings, and estimate assembly (clinical_build_governance); PFM-1.1-05 and PFM-3.1-05 are its process-level instances |
